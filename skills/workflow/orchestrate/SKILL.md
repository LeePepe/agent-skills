...
- 一个切片 = 一个 repo 内、一个 layer 范围、一个目的、能独立 merge 且符合[PR 范围与大小](../README.md#pr-范围与大小)的 PR。
  先列各片 owned 路径、不做项、预计增删行/文件数与依赖；没有合格切片，不派发实现。跨 repo 按 provider → 发布 → 消费者（W5）。
- 依赖只按真实的数据/接口关系排；无依赖的切片并行，但**同一 repo 同一路径只有一个写者**。
- 共享接口、迁移、全局重构串行；机械推广（多仓同一模板）可并行。
- 需求不清先问 Owner（W7）或用 grilling；架构迁移不夹带功能/UX 改动。

完成条件：每片只有一个可独立接受的结果，包含对应测试/必要文档，符合预算且有清楚的合入顺序；一个 layer 内的多件事仍须拆片。
...
派发内容必须包含[交接最小字段](../README.md#交接最小字段)。subagent prompt 里写明：目标 repo 与 base、owned 路径、
“先读该 repo AGENTS.md”、单一目的、预算/不做项、验收、禁止事项、完成时回报 PR URL。
在派发中携带该片的预算和超限退回责任，不假设执行者或部署角色会自动读到个人 skills 的更新。

## 4. 观察

- 以 PR 为观察单位：CI 结果、review comment、是否卡住。实现/检查失败交回**原作者**；范围/规模交 TL；确需新审批、权限或运行边界决策才交 Owner。不开第二个写者。
- 观察整 PR 的规模和 scope 漂移，不只看最近一次 push。超限或出现第二个独立目的，交原 TL/实现者停止扩张并重切；大小问题本身不升级为 Owner 审批。
...
- 切片完成 = 目的/范围/规模复核通过 + PR merged + 验收项在 PR 上有证据（CI、review、必要时截图/消费者 build）。
