...
- 一个切片 = 一个 repo 内、一个 layer 范围、能独立 merge 的 PR。跨 repo 的改动拆成“provider PR → 发布 → 各消费者 pin PR”（W5）。
...
“先读该 repo AGENTS.md”、验收、禁止事项、完成时回报 PR URL。不要假设执行者有你的个人 skills 或本机文件。

## 4. 观察

- 以 PR 为观察单位：CI 结果、review comment、是否卡住。卡住时把问题交回**原 owner**（实现问题→原作者，gate/权限→Owner），不开第二个写者。
...
- 切片完成 = PR merged + 验收项在 PR 上有证据（CI、review、必要时截图/消费者 build）。
